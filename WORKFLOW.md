...
- `AGENT_AUDIT_WATCHLIST.md` — типовые сбои + обязательные preflight/post-check перед каждой клиникой
...
## Мини-checklist перед каждой клиникой (обязательно)

1. Перед запуском проверки пройти `Preflight` из `AGENT_AUDIT_WATCHLIST.md`.
2. Перед сохранением JSON пройти `Post-check` из `AGENT_AUDIT_WATCHLIST.md`.
3. Если найден новый повторяющийся сбой, сразу добавить его в секцию `Новые паттерны` в `AGENT_AUDIT_WATCHLIST.md`.
